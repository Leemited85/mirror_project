import type { ModelAnalyzeRequest, ModelAnalyzeResponse, TryOnRequest, TryOnResponse } from '../types/fitting';
...
export async function requestModelAnalysis(payload: ModelAnalyzeRequest): Promise<ModelAnalyzeResponse> {
  let response: Response;
  try {
    response = await fetch(`${API_BASE_URL}/api/models/analyze`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
  ...
  if (!response.ok) {
    throw new Error(`Model analysis API returned ${response.status}`);
  }
